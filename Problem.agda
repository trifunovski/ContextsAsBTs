...
mutual
  ...
  subProp1 : ∀ {Γ Γ' Δ Δ' n} → Γ ⊢s Γ' → Γ' ≡ Δ' → Δ' ⊢s Δ → Γ' size n → Γ ⊢s Δ
  subProp1 {n = Z} sub1 (emp x x₁) sub2 (s0 x₂) = equiv (emp (emptySubLemma sub1 x) sinE) emptySub (emp sinE (emptySubLemma (symSub sub2) x₁))
  subProp1 {n = Z} sub1 (decom x x₁ eqpf) sub2 (s0 x₂) = abort (lemmaEmptyDecom x₂ x)
  subProp1 {n = S .0} sub1 (emp () x₁) sub2 s1
  subProp1 {n = S n} sub1 (emp x x₁) sub2 (sm x₂ size) = abort (lemmaEmptyDecom x x₂)
  subProp1 {n = S n} sub1 (decom x x₁ eqpf) sub2 size with subLemma2 sub1 x | subLemma2 (symSub sub2) x₁
  ... | Γ1 , dec1 , sub3 | Δ1 , dec2 , sub4 with subProp1 sub3 eqpf (symSub sub4) (decdSize x size)
  ... | sub5 = equiv (decom dec1 SD1 (refl (decdSize dec1 (subSameSize (symSub sub1) size)))) (comma var sub5) (sym (decom dec2 SD1 (refl (decdSize dec2 (subSameSize sub2 (equivSameSize (sym (decom x x₁ eqpf)) size))))))
  ...
  transSub sub1 (equiv x sub3 x₁) size1 size2 size3 = subProp1 sub1 x (equiv (refl (equivSameSize (sym x) size2)) sub3 x₁) size2
  transSub (equiv x sub1 x₁) sub2 size1 size2 size3 = equiv x (transSub sub1 (equiv x₁ sub2 (refl size3)) (equivSameSize (sym x) size1) (equivSameSize x₁ size2) size3) (refl size3)
  ...
  ... | Γ'' , dec4 , eq3 | n , size1 | m , size2 = Γ'' , dec4 , equiv (sym eq3) sub2 (sym eq2)
  ...
  subLemma2 {Δ' = Δ'} (comma sub sub₁) SD1 with subLemma2 sub (SD sinE) | findSize _
  ... | Γ' , dec1 , sub2 | n , size1 with addEmptyCtx (refl (subSameSize (symSub sub₁) size1)) (emptySubLemma sub2 sinE)
  ... | res1 , res2 , res3 , res4 = _ , MD1 dec1 , equiv res2 sub₁ (refl size1)
  subLemma2 {Δ' = Δ'} (comma sub sub₁) SD2 with subLemma2 sub₁ (SD sinE) | findSize _
  ... | Γ' , dec1 , sub2 | n , size1 with addEmptyCtx (refl (subSameSize (symSub sub) size1)) (emptySubLemma sub2 sinE)
  ... | res1 , res2 , res3 , res4 = _ , MD2 dec1 , equiv res1 sub (refl size1)
  ...
  ... | Δ'' , dec4 , eq3 = Δ'' , dec4 , equiv (sym eq3) sub2 (sym eq2)
